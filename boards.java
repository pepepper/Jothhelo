public class boards {
	...
	boolean isPutable(int x, int y, int deltax, int deltay, int turn) {
		x += deltax;
		y += deltay;
	...
	public boolean isPutable(int x, int y, int turn) {
		boolean ret = false;
		if (board[x][y] != 0)
	...
	public boolean Put(int x, int y, int turn) {
		boolean ret = false;
		if (board[x][y] != 0)
			return ret;// 1 white -1 black
		if (isPutable(x, y, 1, 0, turn)) {
	...
	public boolean isFull(int turn) {
		for (int x = 0; x < 16; x++) {
			for (int y = 0; y < 16; y++) {
				if (board[x][y] == 0)
	...
	public boolean freeput(int x, int y, int turn) {
		if (board[x][y] != 0)
			return false;
		int i = 0;
		board[x][y] = turn;
	...
	void Reverse(int x, int y, int vect_x, int vect_y, int turn) {
		int i = 0;
		board[x][y] = turn;
		while (delta[i][0] != 0) {
			i++;
